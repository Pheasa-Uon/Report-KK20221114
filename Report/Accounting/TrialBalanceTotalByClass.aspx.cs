...
namespace Report.Accounting
{
    public partial class TrialBalanceTotalByClass : System.Web.UI.Page
    {
        ...
        protected void btnView_Click(object sender, EventArgs e)
        {
            ...
            {
                sql = "SELECT acc.id,acc.acc_class_id, aac.class_name,CC.currency,IFNULL(acc.parent_gl,acc.gl) parent_gl,(SELECT gl_name FROM acc_chat_of_account WHERE gl=IFNULL(acc.parent_gl,acc.gl)) parent_gl_name,acc.gl,acc.gl_name,TRIM(acc.side) AS side, " +
